...
# ask gpt to turn the kb into pyke facts + rules
...
# split the gpt output into facts list and rules list
def split_kb(raw):
    facts = []
    rules = []
...
# just dump facts into the kfb file, one per line
...
# query with one variable free, get back everything it could be
def ask(engine, pred, known_val):
    ...
    for ns in ["facts", "rules"]:
        ...
        try:
            ...
            # probably just means no rule/fact matched, thats fine
            pass
    return answers
...
def main():
    ...
    print("asking gpt to translate kb...")
    ...
    # wipe old cache so we dont reuse stale stuff
